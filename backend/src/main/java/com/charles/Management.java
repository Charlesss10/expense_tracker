	...
	public boolean authManager(String authManagerPrompt) throws SQLException, IOException {
		...
		switch (authManagerPrompt) {
			...
			case "LOGIN" -> {
				...
				if (token != null) {
					String accountIdStr = authManager.getSessionAccountId(token);
	...
	public void dataStorage() throws SQLException {
		...
		switch (dataChoice) {
			...
			case "B" -> {
				try {
					String CSV_FILE_PATH = "./expense_tracker/transactions.csv";
					dataStorage.loadDataConsole(CSV_FILE_PATH, authManager.getAccountId());
				} catch (IOException ex) {
				}
				break;
			}
	...
	public void expenseTracker() throws SQLException, ClassNotFoundException, IOException {
		...
		if (login == false && authManager.getFailedLoginAttempts() == 3) {
			...
			while (login) {
				...
				System.out.println("A - Transaction Manager");
				System.out.println("B - Generate Report Summary");
				System.out.println("C - Data Storage");
				System.out.println("D - Account Manager");
